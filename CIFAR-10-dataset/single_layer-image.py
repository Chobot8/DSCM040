...
# Define a hypermodel for single-layer NN
def build_model(hp):
    model = Sequential()
...
tuner = kt.RandomSearch(
    ...
    directory='my_dir',
    project_name='cifar10_single_layer_nn'
)
...
plt.ylabel('True')
plt.title('Confusion Matrix - Single-layer NN with Hyperparameter Tuning')
plt.savefig('pictures/confusion_matrix_single_layer_nn_tuned.png')
# plt.show()
...
plt.title('Training and validation loss/accuracy - Single-layer NN with Hyperparameter Tuning')
plt.xlabel('Epochs')
plt.ylabel('Loss/Accuracy')
plt.legend()
plt.savefig('pictures/training_validation_loss_accuracy_single_layer_nn_tuned.png')
# plt.show()

# Save training time information
with open('pictures/training_time_single_layer_nn_tuned.txt', 'w') as f:
    f.write(f"Training time (Single-layer NN with Hyperparameter Tuning): {training_time:.2f} seconds\n")
    f.write(f"Test accuracy (Single-layer NN with Hyperparameter Tuning): {accuracy*100:.2f}%\n")
